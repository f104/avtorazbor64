...
//        $c->prepare(); echo $c->toSQL();
    ...
    public function run() {
        ...
        $cols = [
            'name' => 
                ['title' => $this->lang['item.name'], 'tpl' => '@INLINE <a href="'.$this->uri.'/view?id={$row.id}&'.  http_build_query($this->filters).'">{$row.name}</a>&nbsp;<a href="cars/item?id={$row.id}" target="_blank"><i class="fa fa-external-link"></i></a>'],
            'price' => 
