...
//        public $last_modified;
        ...
        /**  @var Brevis/Components/Form $formAuth Форма авторизации */
        public $formAuth = null;
...
//            $this->last_modified = file_get_contents(PROJECT_LM_FILE);
            ...
            if ((!defined('PROJECT_API_MODE') || !PROJECT_API_MODE) and !$this->core->isAuth) {
                $this->_formAuth();
            }
        }
        ...
        public function getUserMenu() {
            $menu = [
                ...
                2 => [
                    ...
                    'catalog' => [
                        'title' => '<i class="fa fa-book"></i> '.$this->lang['catalog'],
                    ],
                    'eventlog' => [
...
//            }
         ...
         * @param bool $filter Выборка непустых данных для фильтра
         * @return array
         */
        public function getRegions($country_id = 1, $filter = false) {
            $c = $this->core->xpdo->newQuery('Brevis\Model\Region');
            $c->where(['country_id' => $country_id]);
            $c->select($this->core->xpdo->getSelectColumns('Brevis\Model\Region', 'Region', '', ['id', 'name']));
            $c->sortby('name');
            if ($filter) {
                $c->innerJoin('Brevis\Model\User', 'User', ('User.region_id = Region.id'));
                $c->groupby('id');
            }
        ...
        /**
         * Готовит форму авторизации
         */
        private function _formAuth() {
            
            $formAuth = new Form([
                'id' => 'auth_form',
                'class' => 'white-popup-block mfp-hide js-ajaxform',
                'action' => 'user/login',
            ], $this);
            $formAuth->template = ''
                    . '<form {$options}>'
            ...
            $formAuth->button('Отмена', ['type' => 'reset', 'class' => 'btn btn-default']);
            $formAuth->input('returnUri', ['type' => 'hidden'])->setValue($_SERVER['REQUEST_URI']);
            $this->formAuth = $formAuth;
        }
        
        public function makeHauthLoginTpl() {
            $hauth = new \Brevis\Components\Hauth\Hauth($this->core);
            $providers = $hauth->getLoginProviders();
            $tpl = $hauth->getLoginTemplate();
            return $this->template($tpl, [
                ...
                /**
         * Адрес для возврата после логаута
         * Если разрешения для контроллера не пусты - возврат в корень
         * @return string urlencoded строка
         */
        public function getLogoutReturnUri() {
            $uri = '';
            if (empty($this->permissions)) {
